...
public class DatabaseAdapter {
	...
	public void insertData(Context context, ArrayList<Vehicle> dataList) {
		...
		try {
			for (Vehicle data : dataList) {
				...
				values.put(VEH_SPEED, data.getVehicleSpeed());
	...
	public ArrayList<Vehicle> getDataArray(Context context, int position) {
		AppLog.enter(TAG, AppLog.getMethodName());
		DatabaseHelper dbHelper = new DatabaseHelper(context);
		SQLiteDatabase database = dbHelper.getWritableDatabase();
		Cursor cursor = database
				.rawQuery("select * from " + TABLE_UPLOAD, null);

		ArrayList<Vehicle> alData = new ArrayList<Vehicle>();
		Vehicle data = null;
		...
		try {
			...
			if (cursor.getCount() > 0) {
				...
				for (int i = 0; i < cursor.getCount(); i++) {
					...
					data.setLongitude(cursor.getString(cursor
							.getColumnIndex(LONGITUDE)));
					data.setTimeStamp(cursor.getString(cursor
							.getColumnIndex(TIMESTAMP)));
					data.setVehicleId(cursor.getString(cursor
							.getColumnIndex(VEHICLE_ID)));
					data.setVehicleSpeed(cursor.getString(cursor
							.getColumnIndex(VEH_SPEED)));
					data.setIsOffline(cursor.getInt(cursor
							.getColumnIndex(IS_OFFLINE)));
	...
	public ArrayList<Vehicle> getDataArrayByFlag(Context context, int flag) {
		AppLog.enter(TAG, AppLog.getMethodName());
		DatabaseHelper dbHelper = new DatabaseHelper(context);
		SQLiteDatabase database = dbHelper.getWritableDatabase();
		...
		try {
			...
			if (cursor.getCount() > 0) {
				...
				for (int i = 0; i < cursor.getCount(); i++) {
					...
					data.put(RID,
							cursor.getString(cursor.getColumnIndex(RID)));
	...
	public boolean UpdateData(Context context, JSONObject json) {
		...
		try {
			...
			index = database.update(TABLE_UPLOAD, values, "timeStamp=?",
					new String[] { json.getString("timeStamp") });
